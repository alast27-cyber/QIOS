...
io.on('connection', (socket) => {
    ...
    socket.on('find_ai_game', () => {
        ...
        const newSession = {
            ...
            fruitEaten: false,
            isAiGame: true,
            aiSnake: [{x: 5, y: 5}], // Start AI in a different corner
            aiInterval: null
        };
        gameSessions.set(sessionId, newSession);
        connectedNodes.get(player1).sessionId = sessionId;
    ...
    socket.on('ate_food', (data) => {
        ...
        setTimeout(() => {
            if (gameSessions.has(node.sessionId)) { // Check if session still exists
    ...
    socket.on('game_over', () => {
        ...
        if (connectedNodes.has(winnerId)) { // Check if winner is human
...
server.listen(PORT, () => { console.log(`QIOS Back Office is running.`); });const express = require('express');
const http = require('http');
const { Server } = require("socket.io");
const path = require('path');
